...
que puede usarse para resolver un gran número de problemas algorítmicos.
Sin embargo, hay muchos temas relacionados con los árboles de segmentos
que aún no hemos tratado.
Ahora es momento de discutir algunas variantes más avanzadas
de los árboles de segmentos.

Hasta ahora, hemos implementado las operaciones
de un árbol de segmentos recorriendo \emph{de abajo hacia arriba}
en el árbol.
...
Sin embargo, en árboles de segmentos más avanzados,
a menudo es necesario implementar las operaciones
de otra manera, \emph{de arriba hacia abajo}.
Usando este enfoque, la función queda de la siguiente manera:
\begin{lstlisting}
...
y es inicialmente $[0,n-1]$.
Al calcular la suma,
si $[x,y]$ está fuera de $[a,b]$,
la suma es 0,
y si $[x,y]$ está completamente dentro de $[a,b]$,
la suma puede encontrarse en \texttt{tree}.
Si $[x,y]$ está parcialmente dentro de $[a,b]$,
la búsqueda continúa recursivamente hacia la
...
Los nodos grises indican los nodos donde la recursión
se detiene y la suma puede encontrarse en \texttt{tree}.
...
las operaciones toman tiempo $O(\log n)$,
...
Usando \key{propagación perezosa}, podemos construir
un árbol de segmentos que soporta \emph{tanto} actualizaciones de rango
como consultas de rango en tiempo $O(\log n)$.
La idea es realizar actualizaciones y consultas
de arriba hacia abajo y realizar las actualizaciones
de forma \emph{perezosa} de modo que se propaguen
hacia abajo en el árbol solo cuando es necesario.
...
relacionado con el subarreglo correspondiente.
Además, el nodo puede contener información
relacionada con actualizaciones perezosas, que aún no ha sido
propagada a sus hijos.

Hay dos tipos de actualizaciones de rango:
cada valor del arreglo en el rango o bien se
\emph{incrementa} en algún valor
o se le \emph{asigna} algún valor.
Ambas operaciones pueden implementarse usando
ideas similares, e incluso es posible construir
un árbol que soporte ambas operaciones al mismo tiempo.

\subsubsection{Árboles de segmentos perezosos}

Consideremos un ejemplo donde nuestro objetivo es
construir un árbol de segmentos que soporte
dos operaciones: incrementar cada valor en
$[a,b]$ en una constante y calcular la suma de
los valores en $[a,b]$.

Construiremos un árbol donde cada nodo
tiene dos valores $s/z$:
$s$ denota la suma de los valores en el rango,
y $z$ denota el valor de una actualización perezosa,
que significa que todos los valores en el rango
deben incrementarse en $z$.
...
Cuando los elementos en $[a,b]$ se incrementan en $u$,
recorremos desde la raíz hacia las hojas
y modificamos los nodos del árbol de la siguiente manera:
Si el rango $[x,y]$ de un nodo está
completamente dentro de $[a,b]$,
incrementamos el valor $z$ del nodo en $u$ y nos detenemos.
Si $[x,y]$ solo pertenece parcialmente a $[a,b]$,
incrementamos el valor $s$ del nodo en $hu$,
donde $h$ es el tamaño de la intersección de $[a,b]$
y $[x,y]$, y continuamos nuestro recorrido recursivamente en el árbol.

Por ejemplo, la siguiente imagen muestra el árbol después de
incrementar los elementos en $[a,b]$ en 2:
\begin{center}
\begin{tikzpicture}[scale=0.7]
\fill[color=gray!50] (5,0) rectangle (6,1);
\draw (0,0) grid (16,1);
...
También calculamos la suma de los elementos en un rango $[a,b]$
recorriendo el árbol de arriba hacia abajo.
Si el rango $[x,y]$ de un nodo pertenece completamente
a $[a,b]$, agregamos el valor $s$ del nodo a la suma.
De lo contrario, continuamos la búsqueda recursivamente
hacia abajo en el árbol.

Tanto en las actualizaciones como en las consultas,
el valor de una actualización perezosa siempre se propaga
a los hijos del nodo
antes de procesar el nodo.
La idea es que las actualizaciones se propagarán
hacia abajo solo cuando sea necesario,
lo cual garantiza que las operaciones siempre sean eficientes.
...
Nota que a veces es necesario combinar actualizaciones perezosas.
Esto sucede cuando a un nodo que ya tiene una actualización perezosa
se le asigna otra actualización perezosa.
Al calcular sumas, es fácil combinar actualizaciones perezosas,
porque la combinación de las actualizaciones $z_1$ y $z_2$
corresponde a una actualización $z_1+z_2$.

\subsubsection{Actualizaciones polinómicas}

Las actualizaciones perezosas pueden generalizarse de modo que sea
posible actualizar rangos usando polinomios de la forma
\[p(u) = t_k u^k + t_{k-1} u^{k-1} + \cdots + t_0.\]

En este caso, la actualización para un valor
en la posición $i$ en $[a,b]$ es $p(i-a)$.
Por ejemplo, sumar el polinomio $p(u)=u+1$
...
Ahora, la suma de los valores en un rango $[x,y]$ es igual a
\[s+\sum_{u=0}^{y-x} z_k u^k + z_{k-1} u^{k-1} + \cdots + z_0.\]

El valor de tal suma
puede calcularse eficientemente usando fórmulas de suma.
Por ejemplo, el término $z_0$ corresponde a la suma
$(y-x+1)z_0$, y el término $z_1 u$ corresponde a la suma
\[z_1(0+1+\cdots+y-x) = z_1 \frac{(y-x)(y-x+1)}{2} .\]

Al propagar una actualización en el árbol,
los índices de $p(u)$ cambian,
porque en cada rango $[x,y]$,
los valores se
calculan para $u=0,1,\ldots,y-x$.
...
Después de esto, los nodos pueden crearse de la siguiente manera:
\begin{lstlisting}
// create new node
...
el arreglo subyacente es \emph{disperso},
es decir, el rango $[0,n-1]$
de índices permitidos es grande,
pero la mayoría de los valores del arreglo son ceros.
Mientras que un árbol de segmentos ordinario usa $O(n)$ de memoria,
un árbol de segmentos dinámico solo usa $O(k \log n)$ de memoria,
...
$O(\log n)$ nodos,
por lo que cada operación agrega a lo sumo $O(\log n)$
nuevos nodos al árbol.
Así, después de $k$ operaciones, el árbol contiene
a lo sumo $O(k \log n)$ nodos.

Nota que si conocemos todos los elementos que se van a actualizar
...
compresión de índices (Capítulo 9.4).
...
como en un árbol de segmentos ordinario, porque la
estructura completa de cada árbol está almacenada.
También podemos crear nuevos árboles basados en árboles
anteriores y modificarlos de forma independiente.

Considera la siguiente secuencia de actualizaciones,
donde los nodos rojos cambian
y los otros nodos permanecen iguales:
...
permanecen iguales,
por lo que una manera eficiente de almacenar el historial de modificaciones
es representar cada árbol del historial como una combinación
de nuevos nodos y subárboles de árboles anteriores.
En este ejemplo, el historial de modificaciones puede
...
La estructura de cada árbol anterior puede
reconstruirse siguiendo los punteros
que comienzan en el nodo raíz correspondiente.
Como cada operación
...
En lugar de valores individuales, los nodos en un árbol de segmentos
...
tiempo $O(f(n) \log n)$, donde $f(n)$ es
el tiempo necesario para procesar un solo nodo durante una operación.

Como ejemplo, considera un árbol de segmentos que
...
a la que se le puede preguntar cuántas veces cualquier elemento $x$
aparece en el rango correspondiente.
Usando este árbol,
la respuesta a una consulta puede calcularse
combinando los resultados de los nodos
que pertenecen al rango.

Por ejemplo, el siguiente árbol de segmentos
corresponde al arreglo anterior:
\begin{center}
\begin{tikzpicture}[scale=0.7]
...
El árbol usa $O(n \log n)$ de memoria,
...
\end{tikzpicture}
\end{center}
la suma de cualquier subarreglo
puede calcularse
...
toman tiempo $O(\log^2 n)$, porque el árbol grande
y cada árbol pequeño constan de $O(\log n)$ niveles.
El árbol requiere $O(n^2)$ de memoria, porque cada
