...
\begin{itemize}
\item Un \key{camino euleriano} es un camino que
pasa por cada arista exactamente una vez.
\item Un \key{camino hamiltoniano} es un camino
que visita cada nodo exactamente una vez.
\end{itemize}

Aunque los caminos eulerianos y hamiltonianos parecen
conceptos similares a primera vista,
los problemas computacionales relacionados con ellos
son muy diferentes.
Resulta que hay una regla sencilla que
determina si un grafo contiene un camino euleriano,
y también existe un algoritmo eficiente para
encontrar tal camino si existe.
Por el contrario, comprobar la existencia de un camino hamiltoniano es un problema
NP-difícil, y no se conoce ningún algoritmo eficiente para resolverlo.

\section{Caminos eulerianos}

\index{camino euleriano}

Un \key{camino euleriano}\footnote{L. Euler estudió tales caminos en 1736
...
que pasa exactamente una vez por cada arista del grafo.
...
pertenecen a la misma componente conexa y
\begin{itemize}
...
En el segundo caso, los nodos de grado impar son los nodos inicial
y final de un camino euleriano que no es un circuito euleriano.
...
\end{center}
\end{samepage}
los nodos 1, 3 y 4 tienen grado 2,
y los nodos 2 y 5 tienen grado 3.
Exactamente dos nodos tienen grado impar,
así que hay un camino euleriano entre los nodos 2 y 5,
pero el grafo no contiene un circuito euleriano.

En un grafo dirigido,
nos centramos en los grados de entrada y de salida
de los nodos.
Un grafo dirigido contiene un camino euleriano
exactamente cuando todas las aristas pertenecen a la misma
componente conexa y
\begin{itemize}
...
es también un circuito euleriano,
y en el segundo caso, el grafo contiene un camino euleriano
...
\end{tikzpicture}
\end{center}
los nodos 1, 3 y 4 tienen grado de entrada 1 y grado de salida 1,
el nodo 2 tiene grado de entrada 1 y grado de salida 2,
y el nodo 5 tiene grado de entrada 2 y grado de salida 1.
Por tanto, el grafo contiene un camino euleriano
...
El \key{algoritmo de Hierholzer}\footnote{El algoritmo se publicó
en 1873 tras la muerte de Hierholzer \cite{hie73}.} es un método
eficiente para construir
un circuito euleriano.
El algoritmo consta de varias rondas,
cada una de las cuales añade nuevas aristas al circuito.
Por supuesto, suponemos que el grafo contiene
...
algunas (no necesariamente todas) de las aristas del grafo.
Después de esto, el algoritmo extiende el circuito
paso a paso añadiéndole subcircuitos.
El proceso continúa hasta que todas las aristas han sido añadidas
al circuito.

El algoritmo extiende el circuito encontrando siempre
un nodo $x$ que pertenece al circuito pero que tiene
una arista saliente que no está incluida en el circuito.
El algoritmo construye un nuevo camino desde el nodo $x$
que solo contiene aristas que aún no están en el circuito.
Tarde o temprano,
el camino regresará al nodo $x$,
lo que crea un subcircuito.

Si el grafo solo contiene un camino euleriano,
todavía podemos usar el algoritmo de Hierholzer
para encontrarlo añadiendo una arista extra al grafo
y eliminando la arista después de que el circuito
haya sido construido.
Por ejemplo, en un grafo no dirigido,
añadimos la arista extra entre los dos
nodos de grado impar.

A continuación veremos cómo el algoritmo de Hierholzer
construye un circuito euleriano para un grafo no dirigido.
...
\end{center}
\end{samepage}
Después de esto, el algoritmo añade
el subcircuito
...
\end{tikzpicture}
\end{center}
Ahora todas las aristas están incluidas en el circuito,
así que hemos construido con éxito un circuito euleriano.
...
El grafo anterior también tiene un circuito hamiltoniano
...
No se conoce ningún método eficiente para comprobar si un grafo
contiene un camino hamiltoniano, y el problema es NP-difícil.
Aun así, en algunos casos especiales, podemos estar seguros
de que un grafo contiene un camino hamiltoniano.

Una observación sencilla es que si el grafo es completo,
es decir, hay una arista entre todos los pares de nodos,
también contiene un camino hamiltoniano.
También se han obtenido resultados más fuertes:
...
el grafo contiene un camino hamiltoniano.
\item
\index{teorema de Ore}
\key{Teorema de Ore}: %\cite{ore60}
Si la suma de los grados de cada par de nodos no adyacentes
es al menos $n$,
el grafo contiene un camino hamiltoniano.
\end{itemize}
...
si el grafo tiene \emph{un gran número} de aristas.
Esto tiene sentido, porque cuantas más aristas contiene el grafo,
más posibilidades hay de construir un camino hamiltoniano.

\subsubsection{Construcción}

Dado que no hay una forma eficiente de comprobar si existe un camino
hamiltoniano, está claro que tampoco hay un método
para construir eficientemente el camino, porque de lo contrario
podríamos simplemente intentar construir el camino y ver
si existe.

Una forma sencilla de buscar un camino hamiltoniano es
usar un algoritmo de vuelta atrás que recorre todas
las formas posibles de construir el camino.
La complejidad temporal de tal algoritmo es al menos $O(n!)$,
...
de una función $\texttt{possible}(S,x)$,
donde $S$ es un subconjunto de nodos y $x$
es uno de los nodos.
La función indica si hay un camino hamiltoniano
que visita los nodos de $S$ y termina en el nodo $x$.
Es posible implementar esta solución en tiempo $O(2^n n^2)$.
...
exactamente una vez como subcadena, para un alfabeto
fijo de $k$ caracteres.
La longitud de tal cadena es
$k^n+n-1$ caracteres.
Por ejemplo, cuando $n=3$ y $k=2$,
un ejemplo de secuencia de De Bruijn es
\[0001011100.\]
Las subcadenas de esta cadena son todas
las combinaciones de tres bits:
...
y cada arista añade un carácter a la cadena.
...
así que la longitud de la cadena es $k^n+n-1$.
...
de un caballo en un tablero de ajedrez de $n \times n$
...
en caso contrario se llama recorrido \emph{abierto}.

Por ejemplo, aquí hay un recorrido del caballo abierto en un tablero de $5 \times 5$:
...
puede moverse entre las casillas según las reglas del ajedrez.

Una forma natural de construir un recorrido del caballo es usar vuelta atrás.
La búsqueda se puede hacer más eficiente usando
\emph{heurísticas} que intentan guiar al caballo de modo que
...
La \key{regla de Warnsdorf} es una heurística sencilla y eficaz
...
algoritmos polinómicos para encontrar recorridos del caballo
...
La idea es mover siempre el caballo de modo que termine
en una casilla donde el número de movimientos posibles sea lo más
\emph{pequeño} posible.

Por ejemplo, en la siguiente situación, hay cinco
casillas posibles a las que el caballo puede moverse (casillas $a \ldots e$):
\begin{center}
\begin{tikzpicture}[scale=0.7]
\draw (0,0) grid (5,5);
\node at (0.5,4.5) {$1$};
...
\end{tikzpicture}
\end{center}
En esta situación, la regla de Warnsdorf mueve el caballo a la casilla $a$,
porque después de esta elección, solo hay un único movimiento posible.
Las otras elecciones moverían el caballo a casillas donde
