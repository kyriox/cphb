...
En los problemas geométricos, a menudo es un desafío
encontrar una manera de abordar el problema de modo que
la solución al problema pueda implementarse cómodamente
...
\end{tikzpicture}
\end{center}
Una manera de abordar el problema es dividir
...
Esta es una manera posible de resolver el problema,
pero hay una trampa:
...
la recta divisoria está \emph{fuera} del cuadrilátero:
\begin{center}
\begin{tikzpicture}[scale=0.45]
...
\end{tikzpicture}
\end{center}
Sin embargo, otra manera de trazar la recta funciona:
\begin{center}
\begin{tikzpicture}[scale=0.45]
...
\end{tikzpicture}
\end{center}
Para un ser humano es claro cuál de las rectas es la elección
correcta, pero la situación es difícil para una computadora.

Sin embargo, resulta que podemos resolver el problema usando
otro método que es más conveniente para un programador.
En concreto, hay una fórmula general
...
$(x_3,y_3)$ y
$(x_4,y_4)$.
Esta fórmula es fácil de implementar, no hay casos
especiales, e incluso podemos generalizar la fórmula
...
Por ejemplo, $4+2i$ corresponde al
siguiente punto y vector:
...
La clase de números complejos \texttt{complex} de C++ es
útil al resolver problemas geométricos.
...
Además, el código define las macros \texttt{X} e \texttt{Y}
que pueden usarse para referirse a las coordenadas x e y.
...
e imprime sus coordenadas x e y:
...
El siguiente código define los vectores $v=(3,1)$ y $u=(2,2)$,
...
En la práctica,
un tipo de coordenada apropiado suele ser
\texttt{long long} (entero) o \texttt{long double}
(número real).
Es buena idea usar entero siempre que sea posible,
porque los cálculos con enteros son exactos.
Si se necesitan números reales,
deben tenerse en cuenta los errores de precisión
al comparar números.
Una manera segura de verificar si los números reales $a$ y $b$ son iguales
...
La función también puede usarse para
...
y los ángulos disminuyen en sentido horario y aumentan
en sentido antihorario.

La función $\texttt{polar}(s,a)$ construye un vector
cuya longitud es $s$ y que apunta a un ángulo $a$.
Un vector puede rotarse un ángulo $a$
multiplicándolo por un vector de longitud 1 y ángulo $a$.

El siguiente código calcula el ángulo del
vector $(4,2)$, lo rota $1/2$ radianes
en sentido antihorario, y luego calcula el ángulo de nuevo:
...
\section{Puntos y rectas}

\index{producto cruz}

El \key{producto cruz} $a \times b$ de los vectores
...
se multiplican juntos, la coordenada y
del resultado es $x_1 y_2 - x_2 y_1$.

\subsubsection{Ubicación de un punto}

Los productos cruz pueden usarse para comprobar
si un punto está ubicado en el lado izquierdo o derecho
de una recta.
Supón que la recta pasa por los puntos
$s_1$ y $s_2$, estamos mirando desde $s_1$
hacia $s_2$ y el punto es $p$.

Por ejemplo, en la siguiente imagen,
$p$ está en el lado izquierdo de la recta:
\begin{center}
\begin{tikzpicture}[scale=0.45]
...
$p$ está ubicado en el lado izquierdo,
y si el producto cruz es negativo,
$p$ está ubicado en el lado derecho.
Finalmente, si el producto cruz es cero,
los puntos $s_1$, $s_2$ y $p$ están sobre la misma recta.

\subsubsection{Intersección de segmentos}

\index{intersección de segmentos}

A continuación consideramos el problema de comprobar
si dos segmentos
$ab$ y $cd$ se intersecan. Los casos posibles son:

\textit{Caso 1:}
Los segmentos están sobre la misma recta
y se solapan entre sí.
...
En este caso, podemos usar productos cruz para
comprobar si todos los puntos están sobre la misma recta.
Después de esto, podemos ordenar los puntos y comprobar
si los segmentos se solapan entre sí.

\textit{Caso 2:}
Los segmentos tienen un vértice común
...
que no es un vértice de ningún segmento.
...
En este caso, los segmentos se intersecan
exactamente cuando ambos puntos $c$ y $d$ están
en lados diferentes de una recta que pasa por $a$ y $b$,
y los puntos $a$ y $b$ están en lados diferentes
de una recta que pasa por $c$ y $d$.
Podemos usar productos cruz para comprobar esto.

\subsubsection{Distancia de un punto a una recta}

Otra característica de los productos cruz es que
el área de un triángulo puede calcularse
usando la fórmula
\[\frac{| (a-c) \times (b-c) |}{2},\]
donde $a$, $b$ y $c$ son los vértices del triángulo.
Usando este hecho, podemos derivar una fórmula
para calcular la distancia más corta entre un punto y una recta.
Por ejemplo, en la siguiente imagen $d$ es la
distancia más corta entre el punto $p$ y la recta
...
Una manera conveniente de resolver el problema es
enviar un \emph{rayo} desde el punto hacia una dirección arbitraria
y calcular el número de veces que toca
el borde del polígono.
...
De manera correspondiente, los rayos desde $b$
tocan 0 y 2 veces el borde del polígono,
por lo que $b$ está fuera del polígono.

\section{Área de un polígono}

Una fórmula general para calcular el área
de un polígono, a veces llamada la \key{fórmula del cordón de zapato},
es la siguiente: \index{fórmula del cordón de zapato}
\[\frac{1}{2} |\sum_{i=1}^{n-1} (p_i \times p_{i+1})| =
\frac{1}{2} |\sum_{i=1}^{n-1} (x_i y_{i+1} - x_{i+1} y_i)|, \]
Aquí los vértices son
$p_1=(x_1,y_1)$, $p_2=(x_2,y_2)$, $\ldots$, $p_n=(x_n,y_n)$
en un orden tal que
$p_i$ y $p_{i+1}$ son vértices adyacentes en el borde
del polígono,
y el primer y el último vértice son el mismo, es decir, $p_1=p_n$.
...
La idea de la fórmula es recorrer trapecios
cuyo un lado es un lado del polígono,
y otro lado está sobre la recta horizontal $y=0$.
Por ejemplo:
\begin{center}
\begin{tikzpicture}[scale=0.7]
...
El área del polígono es la suma de las áreas de
todos esos trapecios, lo que produce la fórmula
\[|\sum_{i=1}^{n-1} (x_{i+1}-x_{i}) \frac{y_i+y_{i+1}}{2}| =
\frac{1}{2} |\sum_{i=1}^{n-1} (x_i y_{i+1} - x_{i+1} y_i)|.\]

Nota que se toma el valor absoluto de la suma,
porque el valor de la suma puede ser positivo o negativo,
dependiendo de si recorremos en sentido horario o antihorario
a lo largo del borde del polígono.

\subsubsection{Teorema de Pick}

\index{teorema de Pick}

El \key{teorema de Pick} proporciona otra manera de calcular
el área de un polígono siempre que todos los vértices
del polígono tengan coordenadas enteras.
Según el teorema de Pick, el área del polígono es
\[ a + b/2 -1,\]
donde $a$ es el número de puntos enteros dentro del polígono
y $b$ es el número de puntos enteros en el borde del polígono.
...
dos puntos.
La función de distancia usual es la
...
\[|5-2|+|2-1|=4.\]
La siguiente imagen muestra las regiones que están dentro de una distancia de 1
...
Como ejemplo, considera un problema donde se nos dan
$n$ puntos en el plano bidimensional
y nuestra tarea es calcular la máxima distancia de Manhattan
...
\end{tikzpicture}
\end{center}
La máxima distancia de Manhattan es 5
...
es rotar todas las coordenadas 45 grados de modo que
...
rotadas son $p'_1=(x'_1,y'_1)$ y $p'_2=(x'_2,y'_2)$.
Ahora hay dos maneras de expresar la distancia de Manhattan
entre $p_1$ y $p_2$:
\[|x_1-x_2|+|y_1-y_2| = \max(|x'_1-x'_2|,|y'_1-y'_2|)\]

Por ejemplo, si $p_1=(1,0)$ y $p_2=(3,3)$,
las coordenadas rotadas son $p'_1=(1,-1)$ y $p'_2=(6,0)$
y la distancia de Manhattan es
\[|1-3|+|0-3| = \max(|1-6|,|-1-0|) = 5.\]

Las coordenadas rotadas proporcionan una manera simple
...
debemos encontrar dos puntos cuyas
coordenadas rotadas maximicen el valor de
\[\max(|x'_1-x'_2|,|y'_1-y'_2|).\]
Esto es fácil, porque o bien la diferencia horizontal o la vertical
de las coordenadas rotadas tiene que ser máxima.
