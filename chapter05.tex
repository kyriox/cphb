...
La \key{búsqueda completa}
es un método general que puede usarse
para resolver casi cualquier problema de algoritmos.
La idea es generar todas las soluciones
posibles al problema por fuerza bruta,
y luego seleccionar la mejor solución o contar el
número de soluciones, según el problema.
...
pueden necesitarse otras técnicas, como los algoritmos voraces o
...
de un conjunto es usar recursión.
...
($k$ no se incluye en el subconjunto) o la rama derecha
($k$ se incluye en el subconjunto).
...
Como los subconjuntos, las permutaciones pueden generarse
usando recursión.
La siguiente función \texttt{search} recorre
...
\texttt{permutation}.
El arreglo \texttt{chosen} indica qué
...
\texttt{next\_permutation} que puede usarse para esto:
...
Un algoritmo de \key{backtracking} (vuelta atrás)
comienza con una solución vacía
y extiende la solución paso a paso.
La búsqueda recorre recursivamente
todas las formas distintas en que
puede construirse una solución.

\index{problema de las reinas}

Como ejemplo, considera el problema de
calcular el número
de formas en que $n$ reinas pueden colocarse en
...
El problema puede resolverse usando backtracking
colocando reinas en el tablero fila por fila.
Más precisamente, se colocará exactamente una reina
en cada fila de modo que ninguna reina ataque
a ninguna de las reinas colocadas antes.
Se ha encontrado una solución cuando todas las
$n$ reinas han sido colocadas en el tablero.
...
y puede extenderse a una solución completa
colocando dos reinas más en el tablero.
...
El código supone que las filas y columnas
del tablero se numeran de 0 a $n-1$.
Cuando la función \texttt{search} se
llama con el parámetro $y$,
...
El arreglo \texttt{column} lleva registro de las columnas
que contienen una reina,
y los arreglos \texttt{diag1} y \texttt{diag2}
llevan registro de las diagonales.
No se permite agregar otra reina a una
columna o diagonal que ya contiene una reina.
Por ejemplo, las columnas y diagonales del
tablero de $4 \times 4$ se numeran de la siguiente manera:
...
El algoritmo de backtracking anterior
...
en una computadora moderna\footnote{No se conoce ninguna forma de
calcular eficientemente valores más grandes de $q(n)$. El récord actual es
$q(27)=234907967154122528$, calculado en 2016 \cite{q27}.}.

\section{Podar la búsqueda}

A menudo podemos optimizar el backtracking
podando el árbol de búsqueda.
La idea es agregar ''inteligencia'' al algoritmo
para que note tan pronto como sea posible
si una solución parcial no puede extenderse
a una solución completa.
Tales optimizaciones pueden tener un efecto
tremendo en la eficiencia de la búsqueda.

Consideremos el problema
de calcular el número de caminos
en una cuadrícula de $n \times n$ desde la esquina superior izquierda
hasta la esquina inferior derecha tales que el
camino visite cada casilla exactamente una vez.
Por ejemplo, en una cuadrícula de $7 \times 7$,
hay 111712 de tales caminos.
...
En cualquier solución, primero nos movemos un paso
...
Por lo tanto, podemos decidir que siempre nos movemos
primero un paso hacia abajo (o hacia la derecha),
y finalmente multiplicar el número de soluciones por dos.
...
de inmediato si llegamos a la casilla inferior derecha demasiado pronto.
\begin{itemize}
\item
...
así que podemos terminar la búsqueda.
...
Así, el algoritmo se volvió casi 1000 veces
...
y hasta observaciones simples pueden podar
la búsqueda de forma efectiva.
Especialmente útiles son las optimizaciones que
...
El \key{encuentro en el medio} (meet in the middle) es una técnica
donde el espacio de búsqueda se divide en
dos partes de tamaño aproximadamente igual.
Se realiza una búsqueda separada
para cada una de las partes,
y finalmente se combinan los resultados de las búsquedas.

La técnica puede usarse
si hay una forma eficiente de combinar los
resultados de las búsquedas.
En tal situación, las dos búsquedas pueden requerir menos
tiempo que una sola búsqueda grande.
Normalmente, podemos convertir un factor de $2^n$
en un factor de $2^{n/2}$ usando la técnica del encuentro
en el medio.
...
Sin embargo, usando la técnica del encuentro en el medio,
...
Nota que $O(2^n)$ y $O(2^{n/2})$ son complejidades
distintas porque $2^{n/2}$ es igual a $\sqrt{2^n}$.

La idea es dividir la lista en
dos listas $A$ y $B$ tales que ambas
...
elemento de $S_B$ tales que su suma sea $x$.
...
Después de esto, creamos las listas
...
lo que puede hacerse en tiempo $O(2^{n/2})$ usando una técnica similar a la mezcla.
Después de esto, como las listas están ordenadas,
podemos comprobar en tiempo $O(2^{n/2})$ si
la suma $x$ puede crearse a partir de $S_A$ y $S_B$.
