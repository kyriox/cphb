...
tomando siempre la elección que parece
la mejor en ese momento.
Un algoritmo voraz nunca deshace
sus elecciones, sino que construye directamente
la solución final.
Por esta razón, los algoritmos voraces
suelen ser muy eficientes.

La dificultad al diseñar algoritmos voraces
es encontrar una estrategia voraz
que siempre produzca una solución óptima
al problema.
Las elecciones localmente óptimas en un algoritmo
voraz deberían ser también globalmente óptimas.
...
donde se nos da un conjunto de monedas
y nuestra tarea es formar una cantidad de dinero $n$
usando las monedas.
Los valores de las monedas son
$\texttt{coins}=\{c_1,c_2,\ldots,c_k\}$,
y cada moneda puede usarse tantas veces como queramos.
...
hasta que se haya construido la cantidad de dinero requerida.
Este algoritmo funciona en el caso del ejemplo,
...
La corrección del algoritmo puede
demostrarse de la siguiente manera:

Primero, cada moneda 1, 5, 10, 50 y 100 aparece
a lo sumo una vez en una solución óptima,
porque si la
solución contuviera dos de tales monedas,
podríamos reemplazarlas por una moneda y
obtener una solución mejor.
Por ejemplo, si la solución contuviera
las monedas $5+5$, podríamos reemplazarlas por la moneda $10$.

De la misma forma, las monedas 2 y 20 aparecen
a lo sumo dos veces en una solución óptima,
porque podríamos reemplazar
las monedas $2+2+2$ por las monedas $5+1$ y
las monedas $20+20+20$ por las monedas $50+10$.
Además, una solución óptima no puede contener
las monedas $2+2+1$ o $20+20+10$,
porque podríamos reemplazarlas por las monedas $5$ y $50$.

Usando estas observaciones,
podemos mostrar para cada moneda $x$ que
no es posible construir de forma óptima
una suma $x$ o cualquier suma mayor usando solo monedas
que sean menores que $x$.
Por ejemplo, si $x=100$, la mayor suma óptima
usando las monedas más pequeñas es $50+20+20+5+2+2=99$.
Así, el algoritmo voraz que siempre selecciona
...
aunque el algoritmo en sí sea simple.
...
puede resolverse usando algún algoritmo voraz\footnote{Sin embargo, es posible
\emph{comprobar} en tiempo polinómico
...
en algunos casos,
el problema general puede resolverse
de forma eficiente usando un algoritmo de programación
dinámica que siempre da la
respuesta correcta.

\section{Planificación}

Muchos problemas de planificación pueden resolverse
usando algoritmos voraces.
Un problema clásico es el siguiente:
Dados $n$ eventos con sus tiempos de inicio y
fin, encontrar una planificación
...
La primera idea es seleccionar los eventos
tan \emph{cortos} como sea posible.
...
que \emph{comience} lo más \emph{temprano} posible.
...
La tercera idea es seleccionar siempre el siguiente
...
es considerar
qué ocurre si primero seleccionamos un evento
...
nunca puede dar una solución mejor,
y el algoritmo voraz es correcto.

\section{Tareas y fechas límite}

Consideremos ahora un problema donde
se nos dan $n$ tareas con duraciones y fechas límite
y nuestra tarea es elegir un orden para realizar las tareas.
Por cada tarea, ganamos $d-x$ puntos
donde $d$ es la fecha límite de la tarea
...
\end{tabular}
\end{center}
En este caso, una planificación óptima de las tareas
es la siguiente:
\begin{center}
\begin{tikzpicture}[scale=.4]
  \begin{scope}
...
\end{tikzpicture}
\end{center}
En esta solución, $C$ da 5 puntos,
$B$ da 0 puntos, $A$ da $-7$ puntos
y $D$ da $-8$ puntos,
así que la puntuación total es $-10$.

Sorprendentemente, la solución óptima al problema
no depende para nada de las fechas límite,
...
dos tareas una tras otra de modo que la primera tarea
tome más tiempo que la segunda tarea,
podemos obtener una solución mejor si intercambiamos las tareas.
Por ejemplo, considera la siguiente planificación:
\begin{center}
\begin{tikzpicture}[scale=.4]
  \begin{scope}
...
\end{tikzpicture}
\end{center}
Aquí $a>b$, así que deberíamos intercambiar las tareas:
\begin{center}
\begin{tikzpicture}[scale=.4]
  \begin{scope}
    \draw (0, 0) rectangle (4, -1);
...
así que la puntuación total aumenta en $a-b > 0$.
En una solución óptima,
para cualesquiera dos tareas consecutivas,
debe cumplirse que la tarea más corta va
antes que la tarea más larga.
Así, las tareas deben realizarse
ordenadas por sus duraciones.

\section{Minimizar sumas}

A continuación consideramos un problema donde
...
\]
En el caso general, la mejor elección para $x$
...
Este resultado puede deducirse presentando
...
Esta es una parábola que abre hacia arriba
...
de una cadena una \key{palabra de código} que consta de bits.
...
asigna palabras de código a los caracteres
\texttt{A}–\texttt{D}:
\begin{center}
\begin{tabular}{rr}
...
Usando este código, la longitud de la cadena
comprimida es de 18 bits.
Sin embargo, podemos comprimir la cadena mejor
si usamos un código de \key{longitud variable}
donde las palabras de código pueden tener longitudes distintas.
...
la cadena \texttt{AB} o a la cadena \texttt{C}.
...
de la cadena,
y la palabra de código de cada carácter puede leerse
...
Inicialmente, cada carácter de la cadena está
...
El proceso continúa hasta que todos los nodos han sido combinados.
...
Ahora todos los nodos están en el árbol, así que el código está listo.
Las siguientes palabras de código pueden leerse del árbol:
\begin{center}
\begin{tabular}{rr}
carácter & palabra de código \\
\hline
